...
export function Header() {
  ...
  const handleClass = (click) => {
    if (click === path) {
      return "flex flex-col items-center text-lg cursor-pointer border-b border-secondary";
    } else {
      ...
      <nav className="flex gap-4 justify-between items-center mx-5 lg:mx-36 py-7">
        <Link to="/">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl text-white">
              Victor<span className="text-secondary">.dev/</span>
            </h1>
          </div>
        </Link>

        <ul className="lg:flex gap-14 text-white hidden">
          <Link spy={true} smooth={true} to="/" className={handleClass("/")}>
        ...
        <div className="lg:hidden">
          ...
          <button onClick={handleClick} className="fixed top-10 right-5 z-50">
            {click ? (
              <FiAlignRight className="text-3xl text-white cursor-pointer" />
            ) : (
              <FiAlignJustify className="text-3xl text-white cursor-pointer" />
            )}
          </button>
        </div>
      </nav>
